...
class NeRFGUI:
    def __init__(self, opt, trainer, train_loader=None, debug=True):
        ...
        self.mode = 'image' # choose from ['image', 'depth', 'normal_image']
    ...
    def train_step(self):
        ...
        if self.trainer.model.use_sdf:
            print(f"step = {self.step: 5d} (+{self.train_steps: 2d}), loss = {outputs['loss']:.4f}, beta={outputs['beta']}, lr = {outputs['lr']:.5f}")
    ...
    def prepare_buffer(self, outputs):
        ...
        elif self.mode == 'normal_image':
            return outputs['normal_image']
        else:
    ...
    def test_step(self):
        ...
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            suffix = "test_gui_step_{}".format(self.step)
            imageio.imwrite('{}/eval_{}.jpg'.format(self.trainer.workspace, suffix), (np.clip(outputs["normal_image"], 0.0, 1.0) * 255).astype(np.uint8))
    ...
    def register_dpg(self):
        ...
        with dpg.window(label="Control", tag="_control_window", width=400, height=300):
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                dpg.add_combo(('image', 'depth', 'normal_image'), label='mode', default_value=self.mode, callback=callback_change_mode)
