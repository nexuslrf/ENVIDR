...
class _get_scatter_idx(Function):
    @staticmethod
    def forward(ctx, rays, source):
        N = rays.shape[0]
        _backend.get_scatter_idx(rays, N, source)
        return source

get_scatter_idx = _get_scatter_idx.apply
...
class _march_rays_train(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, step_counter=None, mean_count=-1, perturb=False, align=-1, force_all_rays=False, dt_gamma=0, max_steps=1024, early_stop_steps=-1):
        ...
        early_stop_steps = max_steps if early_stop_steps <= 0 else early_stop_steps
        
        _backend.march_rays_train(rays_o, rays_d, density_bitfield, bound, dt_gamma, max_steps, early_stop_steps, N, C, H, M, nears, fars, xyzs, dirs, deltas, rays, step_counter, noises) # m is the actually used points number
...
class _composite_rays_train(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, deltas, rays, T_thresh=1e-4, ret_weights=False, input_alpha=False, accum_deltas=True):
        ...
        _backend.composite_rays_train_forward(sigmas, rgbs, deltas, rays, M, N, T_thresh, accum_deltas, input_alpha, weights_sum, depth, image, weights)

        ctx.save_for_backward(sigmas, rgbs, deltas, rays, weights_sum, depth, image)
        ctx.aux_flags = (accum_deltas, input_alpha)
        ctx.dims = [M, N, T_thresh]
        return weights_sum, depth, image, weights
    
    @staticmethod
    @custom_bwd
    def backward(ctx, grad_weights_sum, grad_depth, grad_image, grad_weights):
        ...
        accum_deltas, input_alpha = ctx.aux_flags
   
        grad_sigmas = torch.zeros_like(sigmas)
        grad_rgbs = torch.zeros_like(rgbs)

        _backend.composite_rays_train_backward(grad_weights_sum, grad_image, grad_depth, sigmas, rgbs, deltas, rays, weights_sum, image, depth, M, N, T_thresh, grad_sigmas, grad_rgbs, accum_deltas, input_alpha)
...
class _composite_rays(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, weights_sum, depth, image, T_thresh=1e-2, input_alpha=False, accum_deltas=True):
        ...
        accum_deltas = int(accum_deltas)
        input_alpha = int(input_alpha)
        _backend.composite_rays(n_alive, n_step, T_thresh, accum_deltas, input_alpha, rays_alive, rays_t, sigmas, rgbs, deltas, weights_sum, depth, image)
        return tuple()
