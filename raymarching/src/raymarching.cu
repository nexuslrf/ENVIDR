...
__global__ void kernel_get_scatter_idx(
    ...
    while (step < num_steps) {
        idx_map[offset + step] = index;
        step ++;
    }
}

void get_scatter_idx(const at::Tensor rays, const uint32_t N, at::Tensor idx_map) {
    static constexpr uint32_t N_THREAD = 128;

    kernel_get_scatter_idx<<<div_round_up(N, N_THREAD), N_THREAD>>>(rays.data_ptr<int>(), N, idx_map.data_ptr<int>());
}
...
__global__ void kernel_march_rays_train(
    ...
    const float bound,
    const float dt_gamma, const uint32_t max_steps, const uint32_t early_stop_steps,
    ...
    while (t < far && num_steps < early_stop_steps) {
        // current point
    ...
    float last_t = near; // replace t0 for more accurate depth
...
void march_rays_train(const at::Tensor rays_o, const at::Tensor rays_d, const at::Tensor grid, const float bound, const float dt_gamma, const uint32_t max_steps, const uint32_t early_stop_steps, const uint32_t N, const uint32_t C, const uint32_t H, const uint32_t M, const at::Tensor nears, const at::Tensor fars, at::Tensor xyzs, at::Tensor dirs, at::Tensor deltas, at::Tensor rays, at::Tensor counter, at::Tensor noises) {

    static constexpr uint32_t N_THREAD = 128;
    
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    rays_o.scalar_type(), "march_rays_train", ([&] {
        kernel_march_rays_train<<<div_round_up(N, N_THREAD), N_THREAD>>>(rays_o.data_ptr<scalar_t>(), rays_d.data_ptr<scalar_t>(), grid.data_ptr<uint8_t>(), bound, dt_gamma, max_steps, early_stop_steps, N, C, H, M, nears.data_ptr<scalar_t>(), fars.data_ptr<scalar_t>(), xyzs.data_ptr<scalar_t>(), dirs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), counter.data_ptr<int>(), noises.data_ptr<scalar_t>());
    }));
}
...
__global__ void kernel_composite_rays_train_forward(
    ...
    const uint32_t accum_deltas,
    const uint32_t input_alpha,
    ...
    while (step < num_steps) {
        ...
        const scalar_t alpha = input_alpha? 0.0f + sigmas[0]: 1.0f - __expf(- sigmas[0] * deltas[0]);
        ...
        t = accum_deltas? t + deltas[1]: deltas[1]; // real delta
        d += weight * t;
...
// image: [N, 3]
template <typename scalar_t>
__global__ void kernel_composite_rays_train_forward_with_weight(
    const scalar_t * __restrict__ sigmas,
    const scalar_t * __restrict__ rgbs,  
    const scalar_t * __restrict__ deltas,
    const int * __restrict__ rays,
...
void composite_rays_train_forward(const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor deltas, const at::Tensor rays, const uint32_t M, const uint32_t N, const float T_thresh, const uint32_t accum_deltas, const uint32_t input_alpha, at::Tensor weights_sum, at::Tensor depth, at::Tensor image, at::Tensor weights) {

    static constexpr uint32_t N_THREAD = 128;
    if (weights.numel()==0) {
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        sigmas.scalar_type(), "composite_rays_train_forward", ([&] {
            kernel_composite_rays_train_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), M, N, T_thresh, accum_deltas, input_alpha, weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>());
        }));
    } else {
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        sigmas.scalar_type(), "composite_rays_train_forward", ([&] {
            kernel_composite_rays_train_forward_with_weight<<<div_round_up(N, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), M, N, T_thresh, accum_deltas, input_alpha, weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>());
        }));
    }
}
...
__global__ void kernel_composite_rays_train_backward(
    ...
    const scalar_t onef = 1.0f, eps = 1e-4f;
    scalar_t r = 0, g = 0, b = 0, ws = 0, t = 0, d = 0;
    ...
    while (step < num_steps) {
        ...
        const scalar_t alpha = input_alpha? 0.0f + sigmas[0]: 1.0f - __expf(- sigmas[0] * deltas[0]);
        const scalar_t weight = alpha * T;
        const scalar_t grad_scale = input_alpha? (onef / (onef - alpha + eps)): deltas[0];
        r += weight * rgbs[0];
        g += weight * rgbs[1];
        b += weight * rgbs[2];
        ws += weight;
...
void composite_rays_train_backward(const at::Tensor grad_weights_sum, const at::Tensor grad_image, const at::Tensor grad_depth, const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor deltas, const at::Tensor rays, const at::Tensor weights_sum, const at::Tensor image, const at::Tensor depth, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor grad_sigmas, at::Tensor grad_rgbs, const uint32_t accum_deltas, const uint32_t input_alpha) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_image.scalar_type(), "composite_rays_train_backward", ([&] {
        kernel_composite_rays_train_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(grad_weights_sum.data_ptr<scalar_t>(), grad_image.data_ptr<scalar_t>(), grad_depth.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), weights_sum.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), M, N, T_thresh, grad_sigmas.data_ptr<scalar_t>(), grad_rgbs.data_ptr<scalar_t>(), accum_deltas, input_alpha);
    }));
}
...
void composite_rays(const uint32_t n_alive, const uint32_t n_step, const float T_thresh, const uint32_t accum_deltas, const uint32_t input_alpha, at::Tensor rays_alive, at::Tensor rays_t, const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor deltas, at::Tensor weights, at::Tensor depth, at::Tensor image) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "composite_rays", ([&] {
        kernel_composite_rays<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh, accum_deltas, input_alpha, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>());
    }));
}